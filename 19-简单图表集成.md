# 第19章：k6 简单集成到图表

## 19.1 概述

k6 提供了多种输出格式，可以轻松地将测试结果可视化。本章介绍如何将 k6 结果集成到简单的图表中，适合快速分析和展示。

## 19.2 JSON 输出

### 基本用法

**运行测试并输出 JSON**：
```bash
k6 run --out json=results.json script.js
```

**输出文件结构**：
```json
{
  "metrics": {
    "http_reqs": {
      "values": {
        "count": 1000,
        "rate": 33.33
      }
    },
    "http_req_duration": {
      "values": {
        "min": 120,
        "max": 450,
        "avg": 234.5,
        "med": 220,
        "p90": 350,
        "p95": 380,
        "p99": 420
      }
    }
  }
}
```

### 使用 Python 可视化

**安装依赖**：
```bash
pip install matplotlib pandas
```

**Python 脚本**：
```python
import json
import matplotlib.pyplot as plt
import pandas as pd

# 读取 JSON 结果
with open('results.json', 'r') as f:
    data = json.load(f)

# 提取指标
metrics = data['metrics']
http_req_duration = metrics['http_req_duration']['values']

# 创建图表
fig, axes = plt.subplots(1, 2, figsize=(12, 5))

# 响应时间分布
labels = ['Min', 'Avg', 'P95', 'P99']
values = [
    http_req_duration['min'],
    http_req_duration['avg'],
    http_req_duration['p95'],
    http_req_duration['p99']
]
axes[0].bar(labels, values, color=['green', 'blue', 'orange', 'red'])
axes[0].set_title('HTTP 请求响应时间')
axes[0].set_ylabel('时间 (ms)')

# 请求速率
http_reqs = metrics['http_reqs']['values']
axes[1].bar(['总请求数', 'QPS'], 
            [http_reqs['count'], http_reqs['rate']],
            color=['blue', 'green'])
axes[1].set_title('请求统计')

plt.tight_layout()
plt.savefig('k6_results.png', dpi=300)
print('图表已保存为 k6_results.png')
```

**运行**：
```bash
python visualize.py
```

## 19.3 CSV 输出

### 基本用法

**运行测试并输出 CSV**：
```bash
k6 run --out csv=results.csv script.js
```

**CSV 格式**：
```csv
metric_name,metric_type,time,value,tags
http_reqs,counter,1234567890,100,
http_req_duration,trend,1234567890,234.5,
http_req_failed,rate,1234567890,0.01,
```

### 使用 Excel 可视化

**步骤**：

1. **导入 CSV**：
   - 打开 Excel
   - 数据 → 从文本/CSV
   - 选择 `results.csv`

2. **创建数据透视表**：
   - 插入 → 数据透视表
   - 行：metric_name
   - 值：value

3. **创建图表**：
   - 选择数据
   - 插入 → 图表
   - 选择图表类型（柱状图、折线图等）

**示例图表类型**：
- **响应时间趋势**：折线图
- **请求分布**：柱状图
- **成功率**：饼图

### 使用 Python pandas 分析

```python
import pandas as pd
import matplotlib.pyplot as plt

# 读取 CSV
df = pd.read_csv('results.csv')

# 筛选 HTTP 响应时间数据
duration_data = df[df['metric_name'] == 'http_req_duration']

# 创建时间序列图
plt.figure(figsize=(12, 6))
plt.plot(duration_data['time'], duration_data['value'])
plt.title('HTTP 请求响应时间趋势')
plt.xlabel('时间')
plt.ylabel('响应时间 (ms)')
plt.grid(True)
plt.savefig('response_time_trend.png')
```

## 19.4 使用在线工具可视化

### 方法 1：使用 Google Sheets

1. **上传 CSV**：
   - 打开 Google Sheets
   - 文件 → 导入 → 上传 `results.csv`

2. **创建图表**：
   - 选择数据
   - 插入 → 图表
   - 自定义图表样式

### 方法 2：使用在线图表工具

**推荐工具**：
- **Chart.js**：https://www.chartjs.org/
- **Plotly**：https://plotly.com/
- **Datawrapper**：https://www.datawrapper.de/

**示例（使用 Chart.js）**：
```html
<!DOCTYPE html>
<html>
<head>
    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
</head>
<body>
    <canvas id="myChart"></canvas>
    <script>
        // 从 JSON 读取数据
        fetch('results.json')
            .then(response => response.json())
            .then(data => {
                const ctx = document.getElementById('myChart').getContext('2d');
                const chart = new Chart(ctx, {
                    type: 'bar',
                    data: {
                        labels: ['Min', 'Avg', 'P95', 'P99'],
                        datasets: [{
                            label: '响应时间 (ms)',
                            data: [
                                data.metrics.http_req_duration.values.min,
                                data.metrics.http_req_duration.values.avg,
                                data.metrics.http_req_duration.values.p95,
                                data.metrics.http_req_duration.values.p99
                            ],
                            backgroundColor: ['green', 'blue', 'orange', 'red']
                        }]
                    },
                    options: {
                        responsive: true,
                        scales: {
                            y: {
                                beginAtZero: true
                            }
                        }
                    }
                });
            });
    </script>
</body>
</html>
```

## 19.5 简单的 Python 脚本分析

### 完整示例脚本

创建 `simple_visualize.py`：

```python
#!/usr/bin/env python3
"""
k6 结果简单可视化脚本
使用方法: python simple_visualize.py results.json
"""

import json
import sys
import matplotlib.pyplot as plt
from datetime import datetime

def load_results(json_file):
    """加载 k6 JSON 结果"""
    with open(json_file, 'r') as f:
        return json.load(f)

def extract_metrics(data):
    """提取关键指标"""
    metrics = data.get('metrics', {})
    
    result = {
        'http_reqs': {
            'count': metrics.get('http_reqs', {}).get('values', {}).get('count', 0),
            'rate': metrics.get('http_reqs', {}).get('values', {}).get('rate', 0)
        },
        'http_req_duration': metrics.get('http_req_duration', {}).get('values', {}),
        'http_req_failed': metrics.get('http_req_failed', {}).get('values', {}).get('rate', 0)
    }
    
    return result

def create_charts(metrics):
    """创建图表"""
    fig, axes = plt.subplots(2, 2, figsize=(15, 10))
    fig.suptitle('k6 性能测试结果', fontsize=16, fontweight='bold')
    
    # 1. 响应时间统计
    duration = metrics['http_req_duration']
    labels = ['Min', 'Avg', 'Med', 'P90', 'P95', 'P99']
    values = [
        duration.get('min', 0),
        duration.get('avg', 0),
        duration.get('med', 0),
        duration.get('p90', 0),
        duration.get('p95', 0),
        duration.get('p99', 0)
    ]
    
    axes[0, 0].bar(labels, values, color=['green', 'blue', 'cyan', 'orange', 'red', 'darkred'])
    axes[0, 0].set_title('HTTP 请求响应时间分布')
    axes[0, 0].set_ylabel('时间 (ms)')
    axes[0, 0].grid(True, alpha=0.3)
    
    # 2. 请求统计
    reqs = metrics['http_reqs']
    axes[0, 1].bar(['总请求数', 'QPS'], 
                   [reqs['count'], reqs['rate']],
                   color=['blue', 'green'])
    axes[0, 1].set_title('请求统计')
    axes[0, 1].set_ylabel('数量')
    axes[0, 1].grid(True, alpha=0.3)
    
    # 3. 成功率
    failed_rate = metrics['http_req_failed']
    success_rate = 1 - failed_rate
    axes[1, 0].pie([success_rate, failed_rate],
                   labels=['成功', '失败'],
                   colors=['green', 'red'],
                   autopct='%1.1f%%',
                   startangle=90)
    axes[1, 0].set_title('请求成功率')
    
    # 4. 响应时间对比（Min vs Max vs Avg）
    axes[1, 1].bar(['Min', 'Avg', 'Max'],
                   [duration.get('min', 0),
                    duration.get('avg', 0),
                    duration.get('max', 0)],
                   color=['green', 'blue', 'red'])
    axes[1, 1].set_title('响应时间对比')
    axes[1, 1].set_ylabel('时间 (ms)')
    axes[1, 1].grid(True, alpha=0.3)
    
    plt.tight_layout()
    return fig

def print_summary(metrics):
    """打印摘要信息"""
    print("\n" + "="*50)
    print("k6 测试结果摘要")
    print("="*50)
    
    reqs = metrics['http_reqs']
    duration = metrics['http_req_duration']
    failed_rate = metrics['http_req_failed']
    
    print(f"\n📊 请求统计:")
    print(f"  总请求数: {reqs['count']:,}")
    print(f"  请求速率: {reqs['rate']:.2f} req/s")
    
    print(f"\n⏱️  响应时间:")
    print(f"  最小值: {duration.get('min', 0):.2f} ms")
    print(f"  平均值: {duration.get('avg', 0):.2f} ms")
    print(f"  中位数: {duration.get('med', 0):.2f} ms")
    print(f"  P95: {duration.get('p95', 0):.2f} ms")
    print(f"  P99: {duration.get('p99', 0):.2f} ms")
    print(f"  最大值: {duration.get('max', 0):.2f} ms")
    
    print(f"\n✅ 成功率:")
    success_rate = (1 - failed_rate) * 100
    print(f"  成功率: {success_rate:.2f}%")
    print(f"  失败率: {failed_rate * 100:.2f}%")
    
    print("="*50 + "\n")

def main():
    if len(sys.argv) < 2:
        print("使用方法: python simple_visualize.py <k6_results.json>")
        sys.exit(1)
    
    json_file = sys.argv[1]
    
    try:
        # 加载数据
        data = load_results(json_file)
        
        # 提取指标
        metrics = extract_metrics(data)
        
        # 打印摘要
        print_summary(metrics)
        
        # 创建图表
        fig = create_charts(metrics)
        
        # 保存图表
        output_file = json_file.replace('.json', '_chart.png')
        fig.savefig(output_file, dpi=300, bbox_inches='tight')
        print(f"✅ 图表已保存为: {output_file}")
        
    except FileNotFoundError:
        print(f"❌ 错误: 找不到文件 {json_file}")
        sys.exit(1)
    except json.JSONDecodeError:
        print(f"❌ 错误: {json_file} 不是有效的 JSON 文件")
        sys.exit(1)
    except Exception as e:
        print(f"❌ 错误: {e}")
        sys.exit(1)

if __name__ == '__main__':
    main()
```

**使用方法**：
```bash
# 运行测试并输出 JSON
k6 run --out json=results.json script.js

# 生成图表
python simple_visualize.py results.json
```

## 19.6 实时监控脚本

### 简单的实时监控

创建 `realtime_monitor.py`：

```python
#!/usr/bin/env python3
"""
k6 实时监控脚本
监控 k6 运行时的指标
"""

import subprocess
import json
import time
import matplotlib.pyplot as plt
from collections import deque
from datetime import datetime

class K6RealtimeMonitor:
    def __init__(self, max_points=100):
        self.max_points = max_points
        self.timestamps = deque(maxlen=max_points)
        self.response_times = deque(maxlen=max_points)
        self.qps = deque(maxlen=max_points)
        
    def parse_k6_output(self, line):
        """解析 k6 输出"""
        try:
            # k6 输出格式: http_reqs=100, http_req_duration=avg=234ms
            if 'http_reqs=' in line and 'http_req_duration=' in line:
                # 简化解析，实际需要根据 k6 输出格式调整
                pass
        except:
            pass
    
    def update_chart(self):
        """更新图表"""
        plt.clf()
        
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 8))
        
        # 响应时间趋势
        if self.timestamps:
            ax1.plot(list(self.timestamps), list(self.response_times), 'b-')
            ax1.set_title('响应时间趋势')
            ax1.set_ylabel('响应时间 (ms)')
            ax1.grid(True)
        
        # QPS 趋势
        if self.timestamps:
            ax2.plot(list(self.timestamps), list(self.qps), 'g-')
            ax2.set_title('QPS 趋势')
            ax2.set_ylabel('请求/秒')
            ax2.set_xlabel('时间')
            ax2.grid(True)
        
        plt.tight_layout()
        plt.pause(0.1)
    
    def monitor(self, k6_process):
        """监控 k6 进程"""
        plt.ion()
        
        try:
            while k6_process.poll() is None:
                # 读取 k6 输出（需要根据实际情况调整）
                time.sleep(1)
                self.update_chart()
        except KeyboardInterrupt:
            print("\n监控已停止")
        finally:
            plt.ioff()
            plt.show()

# 使用示例
if __name__ == '__main__':
    # 启动 k6
    k6_process = subprocess.Popen(
        ['k6', 'run', 'script.js'],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE
    )
    
    # 启动监控
    monitor = K6RealtimeMonitor()
    monitor.monitor(k6_process)
```

## 19.7 总结

本章介绍了 k6 简单图表集成的几种方法：

1. **JSON 输出 + Python**：灵活，可定制
2. **CSV 输出 + Excel**：简单，适合非技术人员
3. **在线工具**：快速，无需安装
4. **Python 脚本**：自动化，可集成

**选择建议**：
- 快速查看：使用 Excel
- 自动化报告：使用 Python 脚本
- 实时监控：使用实时监控脚本
- 分享展示：使用在线工具

---

**下一章**：[第20章：k6 集成到完整监控系统](./20-完整监控系统集成.md)

